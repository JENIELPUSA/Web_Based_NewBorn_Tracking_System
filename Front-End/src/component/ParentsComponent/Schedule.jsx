        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-red-50 to-white dark:from-gray-900 dark:to-gray-800 p-2">
            <div className="w-full max-w-4xl rounded-2xl overflow-hidden shadow-2xl bg-white dark:bg-gray-800">
                ...
                <div className="p-6 space-y-6 relative">
                    {/* Timeline Line: hidden on smaller screens, block on 'xm' and up */}
                    <div className="hidden xm:block absolute left-1/4 -translate-x-1/2 top-0 bottom-0 w-0.5 bg-red-300 dark:bg-red-600"></div>
                        ...
                        currentItems.map((entry, index) => {
                            ...
                            return (
                                <div
                                    ...
                                    // Use 'xm:flex-row' to make it horizontal from 'xm' breakpoint
                                    className="flex flex-col xm:flex-row items-stretch xm:items-center relative group"
                                >
                                    {/* Date Box */}
                                    {/* w-full on small, xm:w-1/4 on xm and up */}
                                    <div className="w-full xm:w-1/4 pr-4 xm:text-right">
                                        ...
                                        {/* Arrow for desktop-like view: hidden on smaller screens, block on 'xm' and up */}
                                        <div
                                            className="hidden xm:block absolute right-0 top-1/2 -mt-2 w-4 h-4 bg-inherit transform rotate-45 translate-x-1/2 -z-10"
                                            style={{ backgroundColor: isToday ? '#b91c1c' : '#dc2626' }}
                                        ></div>
                                    </div>
                                    {/* Content Box */}
                                    {/* w-full on small, xm:w-3/4 on xm and up. No left padding on small, xm:pl-8 on xm and up */}
                                    <div className="w-full xm:w-3/4 pl-0 xm:pl-8 mt-2 xm:mt-0 relative">
                                        {/* Arrow for mobile view: hidden on 'xm' and up, block on smaller */}
