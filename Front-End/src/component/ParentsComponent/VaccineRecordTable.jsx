...
// You might need to import a custom CSS file if you create one
        ...
        <div className="card">
            <div className="card-header flex flex-col gap-4 rounded-t-lg bg-gray-50 p-4 dark:bg-slate-700 xs:flex-row xs:items-center xs:justify-between">
                ...
                <div className="flex flex-col gap-2 xs:flex-row xs:items-center xs:gap-4">
                    {/* DatePicker Container for responsiveness */}
                    <div className="relative w-full xs:w-auto"> {/* Add w-full for small screens, w-auto for larger */}
                        <DatePicker
                            selectsRange
                            startDate={startDate}
                            endDate={endDate}
                            ...
                            // withPortal // Consider removing or conditionally rendering this
                            // Add a custom class for styling the portal content
                            // portalHost={document.getElementById('root')} // If you have a specific portal host element
                            popperClassName="datepicker-portal-responsive" // Apply custom styles to the portal
                        />
                        {(startDate || endDate) && (
                            <button
                                onClick={() => setDateRange([null, null])}
                                className="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 ml-2" // Added margin-left
                            >
                                Clear
                            </button>
                        )}
            ...
            <div className="card-body p-0">
                {/* --- Desktop Table View (visible on xs screens and up) --- */}
                <div className="relative w-full overflow-x-auto hidden xs:block">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="sticky top-0 z-10 bg-gray-100 dark:bg-gray-800">
                            <tr>
                                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">#</th>
                                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Newborn Name</th> {/* Added missing column */}
                                <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mother Name</th> {/* Added missing column */}
                ...
                {/* --- Mobile Card View (visible on screens smaller than xs) --- */}
