...
const NotificationDropdown = ({ notifications, onClose }) => {
    ...
    const normalizedNotifications = combinedNotifications.map((notif, index) => {
        ...
        return {
            ...
            message: typeof notif === "string" ? notif : notif?.message || JSON.stringify(notif) || "Notification",
    ...
    const displayedNotifications = showAllNotifications ? normalizedNotifications : normalizedNotifications.slice(0, 5);
                        ...
                        displayedNotifications.map((notif, index) => (
                            ...
                            >
                                <div className="flex items-center justify-between">
                                        ...
                                        {notif.message.includes("completed vaccines are:") ? (
                                            <>
                                                ...
                                                <span className="font-semibold">
                                                    completed vaccines are: {notif.message.split("completed vaccines are:")[1].trim()}
                                                </span>
                                            </>
                                        ) : (
                                            notif.message
                                    ...
                                    {!notif.readBy?.includes(userId) && <span className="ml-2 inline-block h-2.5 w-2.5 rounded-full bg-red-500" />}
                                </div>
                                <div className="mt-1 text-xs text-gray-400">{notif.createdAt ? formatDate(notif.createdAt) : "No date"}</div>
                            </li>
                        ))
                    )}
                </ul>
