...
const NewbornFormModal = ({ onClose, selectedUser, isOpen }) => {
    ...
    // Assuming your ProfillingContexts provides an UpdateProf function
    const { AddProf, UpdateProf, customError } = useContext(ProfillingContexts);
        const [isSubmitting, setIsSubmitting] = useState(false);

    const [formData, setFormData] = useState({
        _id: "", // This will hold the ID if we are updating an existing profile
        newborn_id: "",
        newborn_name: "",
        blood_type: "",
        health_condition: "",
  ...
  setFormData({
    ...
    // Effect to filter the list of newborns shown in the dropdown based on search input
    useEffect(() => {
    ...
    // Effect to populate form data when selectedUser changes (for update mode)
    useEffect(() => {
        if (selectedUser) {
            // If selectedUser exists, we are in "update" mode
            setFormData({
                _id: selectedUser._id || "",
                newborn_id: selectedUser.newborn_id || "",
                newborn_name: selectedUser.newborn_name || "",
                blood_type: selectedUser.blood_type || "",
                health_condition: selectedUser.health_condition || "",
                notes: selectedUser.notes || "",
            });
        } else {
            // If no selectedUser, we are in "add new" mode, so reset the form
            setFormData({
                _id: "",
                newborn_id: "",
                newborn_name: "",
            ...
            setSearchNewborn(""); // Clear search input for new entry
        }
    }, [selectedUser]);
    ...
    // Specific handler for when a newborn is selected from the dropdown
    const handleSelectNewborn = (newborn) => {
        ...
        setDropdownOpenNewborn(false); // Close dropdown after selection
        setSearchNewborn(`${newborn.firstName} ${newborn.middleName} ${newborn.lastName}`); // Keep selected name in search input
    };

    const handleSave = async () => {
        setIsSubmitting(true);
        const { blood_type } = formData;
        try {
            if (selectedUser) {
                await UpdateProf(formData); // Call the UpdateProf function from context
                setTimeout(() => {
                resetForm();
    ...
    // Determine if the modal is in update mode based on selectedUser prop
                    ...
                    <div className="relative">
                        ...
                        <div
                            className={`flex w-full items-center justify-between rounded-md border px-3 py-2 text-sm ${
                                isUpdateMode
                                    ? "cursor-not-allowed bg-gray-100 dark:bg-gray-700" // Disabled style for update mode
                                    : "cursor-pointer bg-white dark:bg-gray-700" // Enabled style for add mode
                            } text-gray-700 dark:border-gray-600 dark:text-gray-200`}
                            onClick={() => {
                                if (!isUpdateMode) {
                                    // Only allow opening the dropdown in add mode
                                    setDropdownOpenNewborn(!dropdownOpenNewborn);
                                }
                            }}
                        >
                        ...
                        {/* Newborn Search Dropdown Content */}
                                    ...
                                    {filteredNewborns.length > 0 ? (
                                        filteredNewborns.map((nb) => (
                                            <li
                                                ...
                                                onClick={() => handleSelectNewborn(nb)} // Use the new handler
                                            >
